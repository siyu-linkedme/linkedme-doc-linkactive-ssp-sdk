...
## 广告展示规则
LinkActive的SDK或者接口请求广告时，返回的是按照优先级排序的每个广告主对应的广告素材，对于**SDK接入的媒体**：  
SDK会判断该广告素材对应的广告主APP是否安装，如果安装则按照一定优先级轮播广告。
...
## 广告展示规则
LinkActive的接口请求广告时，返回的是按照优先级排序的每个广告主对应的广告素材，为了防止一个设备对同一广告主产生两次拉活，对完成拉活的设备需要做如下处理：  

某条广告被终端用户点击并成功拉活广告主APP后，需要客户端从当次请求的广告列表中<font color="red">删除该条广告，其余广告顺移</font>

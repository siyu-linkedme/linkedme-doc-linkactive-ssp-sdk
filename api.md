...
* 服务端需要对接 [获取广告](https://linkedme.gitbooks.io/linkactive/content/api.html#获取广告接口) 的接口（图中2）
* 服务端处理返回的广告结果（图中3）并回传给客户端（图中4）

## 客户端对接说明
* 客户端在用户访问时，向自己服务器请求广告（图中1）
* 客户端需要对服务端返回的广告（图中4）进行处理之后展示，返回结果处理详见 广告展示规则
* 广告行为发生之后，需要调用 [回调广告行为](/api.md#回调广告行为) 的接口（图中5）通知LinkActive服务端
...
4. 删除已被点击的广告，并将广告列表中的其他广告向前顺移

### 注意事项
* 拿到广告列表后首先需要处理将有效的广告保留下来，将无效的广告主APP对应的广告删除
